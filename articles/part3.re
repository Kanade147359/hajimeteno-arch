...
因みにデスクトップ環境は最低限plasma-desktopをインストールすれば動きます。しかし、ネットワーク設定、サウンド設定など欲しい機能が何もない形だけのGUI環境が出来上がり、どの部分になんのパッケージが必要なのかが分かりにくいので
パッケージグループでまとめて入れてしまいます。

qt6-multimedia-backendは1、jackは2を選びます。ttf-fontは好みなのですが、noto-fontsを選びます。
...
さて、仮想コンソールに入ったのでpacmanでYakuakeをインストールしましょう。
...
まず、日本語フォントをインストールしていきます。とりあえずGoogleのNoto CJKフォントをインストールしていきます。
...
Arch Linux wikiを参照してみてください@<fn>{fonts}。
システムのフォントもその中から選ぶことができるため、後に好きなように変更をしましょう。

//footnote[fonts][https://wiki.archlinux.org/title/Fonts]
...
//indepimage[pacman4console][コンソール上でpacmanが遊べます][scale=0.7]
...
//indepimage[editapp][アプリケーションを編集][scale=0.7]
