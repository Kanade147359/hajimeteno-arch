...
@<href>{https://github.com/TechBooster/C89-FirstStepReVIEW-v2/blob/master/articles/review-introduction.re}より引用
...
@<code>{title}は、その章のキャプションになります（@<title>{article}）。

@<code>{chap}は、その章の見出し番号になります（@<chap>{article}）。

@<code>{chapref}は、その章の見出し番号とキャプションになります（@<chapref>{article}）。

これらの構文の参照先には、Re:VIEWの章の識別子を指定します。

章の識別子は、Re:VIEWの章のファイル名から拡張子@<tt>{.re}を取り除いた名前です。
たとえば、@<tt>{article.re}の章を参照する場合、@<code>{article}が識別子になります。
