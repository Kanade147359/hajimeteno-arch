...
//indepimage[linux-kernel][Linux Kernel Archive][scale=0.55]
...
//indepimage[Red_Hat][Red Hatのロゴ][scale=0.7]
...
//indepimage[Arch][Arch Linuxのロゴ][scale=0.5]
 ...
 パッケージマネージャはpacmanです。
 Arch Linuxは自由さこと魅力ですが、システムのメンテナンスや構築が面倒です。
 Arch系のディストリビューションはその面倒な部分を改善することを目標とするディストリビューションが多い気がします。
 Arch Linux公式リポジトリを参照できることが大きな強みです。
 また、ローリングリリースであることも大きな特徴です。
...
Linuxを素に近い状態からカスタマイズできるので、Linuxの学習にはもってこいなディストリビューションです。
...
Linuxディストリビューションの情報がまとまっているサイト@<fn>{distrowatch}があります。

//footnote[distrowatch][https://distrowatch.com/]
